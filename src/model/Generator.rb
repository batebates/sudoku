class Generator
	...
	def initialize(difficulty, grid_string = "")
		...
		if grid_string != ""
			...
			@gridCorrect = grid_string.split("").map { |str| str.to_i }
			...
			until (obviouses.length <= 5) && (hints <= 40)
				# Assign a random cell to 0, as well as its opposite
				copy = self.dup
		...
		while (hints >= 40-(difficulty*5)) do
			@grid.each_with_index { |value, index|
				if value != 0 && random.rand(0..10)==1 then
					@grid[index] = 0
				end
			}
		end
	...
	def generate()
		...
		0.upto 8 do |row|
			0.upto 8 do |col|
				...
				#@gridCorrect[row*9 + col] = possible(row, col).sample
			end
		end
	end
